...
function concatenateArrays() {
    ...
    return arrays.reduce(function (acc, curr) { return __spreadArray(__spreadArray([], acc, true), curr, true); }, []);
}
...
function getMostExpensiveProduct(products) {
    if (products.length === 0)
        return null;
    return products.reduce(function (acc, curr) { return (curr.price > acc.price ? curr : acc); }, products[0]);
}
var products = [
    { name: "Pen", price: 10 },
...
squareAsync(4).then(function (res) { return console.log(res); }); // Output after 1s: 16
squareAsync(-3).catch(function (err) { return console.error(err); }); // Output: Error: Negative number not allowed
